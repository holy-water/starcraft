...
public class BuildOrderQueue {
	...
	/// lowest priority 인 BuildOrderItem은 front 에, highest priority 인 BuildOrderItem
	/// 은 back 에 위치하게 합니다
	...
	/// BuildOrderQueue에 해당 type 의 Item 이 몇 개 존재하는지 리턴한다. queryTilePosition 을 입력한
	/// 경우, 건물에 대해서 추가 탐색한다
			...
			if (queryType.isUnit() && item.isUnit()) {
				...
				if (item.getUnitType() == queryType.getUnitType()) {
					if (queryType.getUnitType().isBuilding() && queryTilePosition != TilePosition.None) {
						if (itemPosition.getDistance(new TilePosition((int) seedPositionPoint.getX(),
								(int) seedPositionPoint.getY())) <= maxRange) {
							itemCount++;
						}
					} else {
						itemCount++;
	...
	public boolean canSkipCurrentItem() {
		...
		Object[] tempArr = queue.toArray();
		// boolean highestNotBlocking = !((BuildOrderItem)tempArr[queue.size() - 1 -
		// numSkippedItems]).blocking;
	...
	/// 빌드오더를 가장 높은 우선순위로 buildQueue 에 추가한다. blocking (다른 것을 생산하지 않고, 이것을 생산 가능하게 될
	/// 때까지 기다리는 모드) 기본값은 true 이다
	...
	public void queueAsHighestPriority(UnitType unitType, BuildOrderItem.SeedPositionStrategy seedPositionStrategy,
			boolean blocking) {
		int newPriority = highestPriority + defaultPrioritySpacing;
	...
	/// 빌드오더를 가장 낮은 우선순위로 buildQueue 에 추가한다. blocking (다른 것을 생산하지 않고, 이것을 생산 가능하게 될
	/// 때까지 기다리는 모드) 기본값은 true 이다
	...
	public void queueAsLowestPriority(MetaType metaType, BuildOrderItem.SeedPositionStrategy seedPositionStrategy,
			boolean blocking) {
	...
	public void removeCurrentItem() {
		...
		// System.out.println("BuildOrderQueue currentItem to remove is " +
