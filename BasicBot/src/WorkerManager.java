...
public class WorkerManager {
		...
		for (Unit worker : workerData.getWorkers()) {
			...
			if (worker.isUnderAttack()) {
				...
				// firstExpansionLocation
					...
					workerData.setWorkerJob(worker, WorkerData.WorkerJob.RunAway,
							mainBaseLocation.getStaticMinerals().get(0));
				}
			...
			// 새 임무를 지정한다
			if (worker.isIdle()) {
				// workerData 에서 Build / Move / Scout / RunAway 로 임무지정한 경우,
				// worker 는 즉 임무 수행 도중 (임무 완료 전) 에 일시적으로 isIdle 상태가 될 수 있다
	...
	// BasicBot 1.1 Patch Start ////////////////////////////////////////////////
	// 일꾼 탄생/파괴 등에 대한 업데이트 로직 버그 수정 : onUnitShow 가 아니라 onUnitComplete 에서 처리하도록
	// 수정
